...
parent::__construct();
	...
	$id = $_GET['id'];
		...
		$query = "SELECT * FROM `coconatsu` WHERE `date_a`=\"$id\" AND `id`<93";
		
		$result = mysql_query($query) or die ('Error in query');


		while($row = mysql_fetch_array($result)){
			$temp = new Messagecocodel($row['date_a'],$row['sub'],$row['context'],$row['pic'],$nzm);
		...
		while($row = mysql_fetch_array($result)){
			echo "<input type=\"button\" value=\"下一篇\n(".$row['date_a'].")\" style=\"width:120px;height:70px;font-size:20px;\" onclick=\"location.href='show_coconatsu.php?id=".$row['date_a']."'\"><br>";
			echo "<br>";
...
echo "<div style=\"width: 330px; height: 38px;cursor: pointer; background: url(source/top21.png);\"
    title=\"TOP\"
    onClick=\"window.open('coconatsu.php','_self');\"
...
<div style="width: 330px; height: 38px;cursor: pointer; background: url(source/top21.png);"
    title="TOP"
    onClick="window.location='coconatsu.php';"
    onmouseover="this.style.background ='url(source/top22.png)';"
    onmouseout="this.style.background='url(source/top21.png)';">
</div>
