...
<div style="width: 101px; height: 37px;cursor: pointer; background: url(source/top1.png);"
    title="TOP"
    onClick="window.location='2015.php';"
...
parent::__construct();
	...
	$id = $_GET['id'];
		...
		$query = "SELECT * FROM `hinabita` WHERE `id`>518 AND `id`<1086 AND `date_a`=\"$id\"";
		
		$result = mysql_query($query) or die ('Error in query');


		while($row = mysql_fetch_array($result)){
			$temp = new Message2015del($row['date_a'],$row['sub'],$row['context'],$row['pic'],$nzm);
		...
		while($row = mysql_fetch_array($result)){
			echo "<input type=\"button\" value=\"下一篇\n(".$row['date_a'].")\" style=\"width:120px;height:70px;font-size:20px;\" onclick=\"location.href='show2015.php?id=".$row['date_a']."'\"><br>";
			echo "<br>";
			}
		mysql_close($con);
